...
func handleGroups(vars map[string]interface{}, groups map[string]interface{}, isProjectAware bool, projects map[string]interface{}) error {
	if err := handleProjectNameInput(isProjectAware); err != nil {
	...
	for _, k := range keys {
		...
		if handleVariables {
			...
			if err := handleInputValues2(groupVars, isProjectAware, projects); err != nil {
				return shared.CheckForErrors(err)
			}
		}
	}

	logWrapper.Println("Completed 'handleGroups'")
	return nil
}
...
func handleInputValues2(vars map[string]interface{}, isProjectAware bool, projects map[string]interface{}) error {
	...
	logWrapper.Println("Completed 'handleInputValues2'")
	return nil
}
