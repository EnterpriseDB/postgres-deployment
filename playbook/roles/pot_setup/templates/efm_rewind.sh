...
###################################################################################
readonly PGENGINE='{{ pg_bin_path }}'
readonly PG_CTL="${PGENGINE}/pg_ctl"
readonly PSQL="${PGENGINE}/psql -qAt"
readonly PG_REWIND="${PGENGINE}/pg_rewind"
readonly PGDATA="{{ pg_data }}"

readonly PGUSER="{{ pg_replication_user }}"
readonly PGDATABASE="{{ pg_database }}"
readonly PGPORT="{{ pg_port }}"
...
###################################################################################
CURRENT_IP=$(ifconfig -a|grep inet|head -n1|awk '{print $2}')
CURRENT_HOST=$(grep ${CURRENT_IP} /etc/hosts|awk '{print $2}')
...
###################################################################################
# create a standby.signal file and update the primary slot and conninfo
###################################################################################
PRIMARY_CONNECTION="user=${PGUSER} passfile=''/var/lib/edb/.pgpass''"
